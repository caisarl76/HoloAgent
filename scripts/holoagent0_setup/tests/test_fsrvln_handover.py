...
def test_stage_validation_or_write_failure_never_publishes_terminal(
    contract, monkeypatch, tmp_path
):
    paths = _fake_paths(tmp_path)
    documents = _pass_documents(paths)
    scenarios = ("validation", "write")
    for scenario in scenarios:
        run_directory = tmp_path / scenario
        run_directory.mkdir(mode=0o700)
        stage_documents = {name: dict(document) for name, document in documents.items()}
        if scenario == "validation":
            stage_documents[ASSET_FILE]["unreviewed"] = True
            expected_error = ContractError
        else:
            real_write = evidence_module.atomic_write_json_no_replace

            def fail_asset(path, document, **kwargs):
                if path.name == ASSET_FILE:
                    raise OSError("injected asset write failure")
                return real_write(path, document, **kwargs)

            monkeypatch.setattr(
                evidence_module, "atomic_write_json_no_replace", fail_asset
            )
        ...
        monkeypatch.setattr(
            ...
            evidence_module.atomic_write_json_no_replace
            if scenario == "validation"
            else real_write,
        )
